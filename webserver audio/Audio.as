...
//		ignore delay (seconds) -- don't play again for x seconds
	...
	public class Audio
	{
		...
		private function loadAudioConfig()
		{
			...
			//For loading from a server use:
			//audioConfigLoader.load(new URLRequest("http://www.newgrounds.com/dump/download/f69d614da866a08cff0d4e40f4eb4d8f"));
			//Need to add a <URL> tag that the audio loader looks for
		...
		private function loadResidentAudio()
		{
			...
			{
				//When loading from webserver, replace var path = "data/audio/" + sampleConfig[key].file;
				//with var path = sampleConfig[key].URL;
